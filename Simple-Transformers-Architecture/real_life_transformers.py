...
# Create masks
src_mask = (src_tensor != eng_word2idx['<pad>']).unsqueeze(1).unsqueeze(2)
tgt_mask = (tgt_tensor != fra_word2idx['<pad>']).unsqueeze(1).unsqueeze(2)
tgt_mask = tgt_mask & torch.tril(torch.ones((tgt_tensor.size(1), tgt_tensor.size(1)), device=tgt_tensor.device)).bool()

# Forward pass
output = transformer(src_tensor, tgt_tensor[:, :-1], src_mask, tgt_mask[:, :, :-1, :-1])
...
def tensor_to_sentence(tensor, idx2word):
    return ' '.join([idx2word[idx.item()] for idx in tensor if idx.item() not in [0, 1, 2]])

# Print results
fra_idx2word = {idx: word for word, idx in fra_word2idx.items()}
for i in range(len(eng_sentences)):
    print(f"English: {eng_sentences[i]}")
    print(f"French (actual): {fra_sentences[i]}")
    print(f"French (predicted): {tensor_to_sentence(predicted[i], fra_idx2word)}")
    print()
